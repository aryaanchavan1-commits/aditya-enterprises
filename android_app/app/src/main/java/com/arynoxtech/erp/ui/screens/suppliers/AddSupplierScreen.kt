...
fun AddSupplierScreen(
    ...
    LaunchedEffect(supplierId) { if (supplierId != null) viewModel.loadSupplier(supplierId) }
    LaunchedEffect(formState.success) { if (formState.success) onNavigateBack() }

    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text(if (supplierId != null) "Edit Supplier" else "Add Supplier", style = MaterialTheme.typography.titleLarge, fontWeight = FontWeight.SemiBold) },
        ...
        Column(
            ...
            OutlinedTextField(value = formState.contactPerson, onValueChange = { viewModel.updateField("contactPerson", it) }, label = { Text("Contact Person") }, modifier = Modifier.fillMaxWidth(), singleLine = true, shape = RoundedCornerShape(12.dp))
            ...
            OutlinedTextField(value = formState.alternatePhone, onValueChange = { viewModel.updateField("alternatePhone", it) }, label = { Text("Alternate Phone") }, modifier = Modifier.fillMaxWidth(), singleLine = true, shape = RoundedCornerShape(12.dp), keyboardOptions = KeyboardOptions(keyboardType = KeyboardType.Phone))
            ...
            Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                OutlinedTextField(value = formState.gstin, onValueChange = { viewModel.updateField("gstin", it) }, label = { Text("GSTIN") }, modifier = Modifier.weight(1f), singleLine = true, shape = RoundedCornerShape(12.dp))
                OutlinedTextField(value = formState.pan, onValueChange = { viewModel.updateField("pan", it) }, label = { Text("PAN") }, modifier = Modifier.weight(1f), singleLine = true, shape = RoundedCornerShape(12.dp))
            }
