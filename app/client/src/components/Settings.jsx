...
export default function Settings() {
  ...
  const detectScanners = async () => {
    try {
      ...
      const d = await r.json();
      if (d.success) {
        setDetectedScanners(d.data);
        setScannerStatus(d.data.length > 0 ? `${d.data.length} scanner(s) detected` : 'No scanners found (keyboard wedge scanners work automatically)');
        showToast(d.data.length > 0 ? 'Scanners detected' : 'Check connections', d.data.length > 0 ? 'success' : 'error');
      }
      ...
      <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:16}}>
        ...
        <div className="card">
          ...
          <div style={{marginBottom:12}}>
            ...
            {scannerStatus && <span style={{fontSize:12, marginLeft:8, color: scannerStatus.includes('No')?'#e74c3c':'#27ae60'}}>{scannerStatus}</span>}
          </div>
