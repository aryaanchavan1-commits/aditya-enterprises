...
export default function BarcodeManager() {
  ...
  const [mode, setMode] = useState('generate'); // generate, scan, camera, history
  ...
  return (
    <div>
                        ...
                        {p.barcode_image && (
                          <button className="btn btn-sm btn-success" onClick={() => {
                            ...
                            printWindow.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><style>
                              ...
                              .price { font-size:13px; font-weight:bold; margin:4px 0; }
                              .barcode-img { max-width:58mm; height:auto; }
                              .code { font-size:9px; font-family:monospace; color:#555; margin:4px 0; }
                            </style></head><body>
                              <h4>${p.name}</h4>
                              <img class="barcode-img" src="${p.barcode_image}" />
                              <div class="code">${p.barcode}</div>
                              <div class="price">Rs.${Number(p.sell_price).toLocaleString('en-IN')}</div>
                              <p style="font-size:8px;color:#999;">Aditya Enterprises</p>
      ...
      {mode === 'camera' && (
        <CameraScanner onScan={(code) => { handleScan(code); setMode('scan'); }} onClose={() => setMode('scan')} />
      )}
              ...
              {products.filter(p => p.barcode_image).map(p => (
                <div key={p.id} className="card" style={{textAlign:'center',padding:12}}>
                  ...
                  <button className="btn btn-sm btn-success" style={{marginTop:6}} onClick={() => {
                    const printWindow = window.open('', '_blank', 'width=400,height=300');
                    printWindow.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><style>
                      body { margin:0; padding:8px; font-family:Arial; text-align:center; width:58mm; }
                      @page { margin:0; size:58mm auto; }
                      h4 { font-size:10px; margin:3px 0; }
                      .price { font-size:12px; font-weight:bold; margin:3px 0; }
                      .barcode-img { max-width:58mm; height:auto; }
                      .code { font-size:8px; font-family:monospace; color:#555; margin:3px 0; }
                    </style></head><body>
                      <h4>${p.name}</h4>
                      <img class="barcode-img" src="${p.barcode_image}" />
                      <div class="code">${p.barcode}</div>
