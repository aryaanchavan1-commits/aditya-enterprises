...
const agentTools = [
  { type: 'function', function: { name: 'get_sales_report', description: 'Get daily or monthly sales report data', parameters: { type: 'object', properties: { period: { type: 'string', enum: ['today', 'this_month', 'yesterday', 'last_month'] }, format: { type: 'string', enum: ['json', 'pdf'] } }, required: ['period'] } } },
  { type: 'function', function: { name: 'get_inventory_status', description: 'Get current inventory status including low stock alerts', parameters: { type: 'object', properties: {}, required: [] } } },
  { type: 'function', function: { name: 'create_product', description: 'Create a new product in inventory', parameters: { type: 'object', properties: { name: { type: 'string' }, quantity: { type: 'number' }, sell_price: { type: 'number' }, hsn_code: { type: 'string' }, description: { type: 'string' }, discount_percent: { type: 'number' } }, required: ['name', 'quantity', 'sell_price'] } } },
  { type: 'function', function: { name: 'get_customer_analysis', description: 'Get customer analysis with repeat customer detection', parameters: { type: 'object', properties: {}, required: [] } } },
  { type: 'function', function: { name: 'get_gst_summary', description: 'Get GST tax summary for filing', parameters: { type: 'object', properties: { month: { type: 'string', description: 'Month in YYYY-MM format' } }, required: [] } } },
];
  ...
  switch (name) {
    case 'get_sales_report': {
      ...
      if (args.period === 'today') {
        const sales = await all("SELECT * FROM sales WHERE sale_date = ? ORDER BY created_at DESC", [date]);
      ...
      if (args.period === 'this_month') {
        const sales = await all("SELECT * FROM sales WHERE sale_date >= ? ORDER BY created_at DESC", [firstOfMonth]);
        const total = sales.reduce((s, x) => s + x.grand_total, 0);
    ...
    case 'create_product': {
      ...
      await run(`INSERT INTO products (name, quantity, sell_price, hsn_code, description, discount_percent, serial_number, barcode) VALUES (?,?,?,?,?,?,?,?)`,
    ...
    case 'get_customer_analysis': {
      const sales = await all("SELECT * FROM sales ORDER BY sale_date");
      const custMap = {};
      sales.forEach(s => {
...
const CHAT_SYSTEM = `You are the AI assistant for Aditya Enterprises ERP 2026. You help with:
...
const AGENT_SYSTEM = `You are an AUTONOMOUS AI AGENT for Aditya Enterprises ERP 2026. You CAN and SHOULD:
...
- If unsure, ask with specific options
...
router.post('/analyze', async (req, res) => {
  try {
    ...
    if (type === 'inventory') {
      ...
      const ts = (await get("SELECT COALESCE(SUM(grand_total),0) as c FROM sales WHERE sale_date = ?", [today])).c;
      const ms = (await get("SELECT COALESCE(SUM(grand_total),0) as c FROM sales WHERE sale_date >= ?", [new Date(new Date().getFullYear(), new Date().getMonth(), 1).toISOString().split('T')[0]])).c;
      const sc = (await get('SELECT COUNT(*) as c FROM sales')).c;
      context = `Sales: ${sc} total, today: Rs.${ts}, this month: Rs.${ms}`;
    } else if (type === 'customers') {
      const sales = await all("SELECT customer_name, COUNT(*) as visits, SUM(grand_total) as spent FROM sales GROUP BY customer_name HAVING visits > 1 ORDER BY spent DESC LIMIT 10");
